...
function buildStoryScenes() {
  ...
  // Кадр 8: после реплики Миши — два квиза на упорядочивание подряд (рейтинги сериалов).
  ...
  if (s14_8) {
    ...
    // Первый квиз ведёт ко второму (тот же фон, без озвучки — появляется сразу).
    s14_8.nextSceneId = 's14_8b';
    const at = scenes.findIndex((scene) => scene.id === 's14_8');
